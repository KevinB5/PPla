...
(defun index (shifts shift)
  (position shift shifts :test #'equal)
  )
    ...
    (loop for shift in (state-shifts auxState)
      do(
        if (and (equal (first (first (state-unusedTasks auxState))) (lastPlace shift))
                (equal (nth 3 (first (state-unusedTasks auxState))) (lastTime shift)))
...
(defun faz-afectacao (tasks strategy)
  ...
  ; (print "s4")
  ; (print s4)
  ; (setf s5 (addTask s4 1))
  ; (print "s5")
  ; (print s5)
  ; (print (state-shifts s5))
  ; (loop for shift in (state-shifts s5)
  ;   do(print (lastPlace shift))
  ;   )
  ; (setf s6 (addTask s5 0))
  ; (print "s6")
  ; (print s6)
  ; (setf s7 (add? s6))
  ; (print (state-shifts s7))
  )
...
(faz-afectacao problem1 "largura" )
